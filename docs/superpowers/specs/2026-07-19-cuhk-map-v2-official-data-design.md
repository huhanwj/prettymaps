...
**关键发现**：CUHK 官方校园地图的位置数据库 `cuhk_location_db.js`（`CUHK_MAP_DATA`，397KB）可直接下载，含 159 栋官方建筑（中英名+坐标）、382 条设施、19 条校巴线路（含官方颜色与折线）、51 个校巴站、26 个官方地标、2 条官方步行捷径（encoded polyline）。校园内数据以它为准；校外（大学站周边、赤泥坪、科学园、码头）继续用 OSM。
...
- `validate.py` 新增下限：official_buildings ≥150（官方实测 159 栋，留缓冲）、shuttle_routes ≥10、walking ≥2
