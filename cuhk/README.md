...
- 改地点：编辑 `cuhk/data/pois.yml`（44 条中英双语 POI），重跑管线；带 `official_name` 的条目会校验官方中英配对
...
- **体育场地**：运动场、跑道与体育中心从普通白色绿地图层中独立显示
- **天桥与楼梯**：只使用官方 PDF 校准的 12 条连接线；实心蓝线表示天桥，蓝色虚线表示楼梯或明显高差通道
- **校巴线路**：可关闭、显示全部或筛选单条官方线路；线路经过每个所属停站并显示行驶方向箭头
- **可选地形**：默认关闭；“地形”开关显示浅色高程分层、等高线及 `0 / 50 / 100 / 150 m` 图例
